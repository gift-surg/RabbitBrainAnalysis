...
pfo_resulting_images_folder = '/Users/sebastiano/Dropbox/RabbitEOP-MRI/docs/Atlas_Paper/images/f1_preview'
...
def create_cuts_on_segmentation(im_segm, direction, intercepts, offset, dtype=np.int):
    """
    ...
    where_segmentation = where_segmentation.astype(np.int) * 255
...
def segmentation_disjoint_intersections(im_segm1, im_segm2):
...
def get_zebra_midplane(im_segm, direction, intercepts, offset, foreground=254, background=255):
    ...
    if direction.lower() == 'axial':
        ...
        for j in range(1, len(intercepts) - 1):
            midplane[:, :, intercepts[j]:intercepts[j]+offset] = \
                foreground * np.ones_like(midplane[:, :, intercepts[j]:intercepts[j]+offset])
        ...
        midplane[:, :, int(midplane.shape[2]/2)] = background * np.ones_like(midplane[:, :, int(midplane.shape[2]/2)])
        for j in range(1, len(intercepts) - 1):
            midplane[:, intercepts[j]:intercepts[j]+offset, :] = \
                foreground * np.ones_like(midplane[:, intercepts[j]:intercepts[j]+offset, :])
    else:
...
if False:
    # create contour
    lm = LM()
    lm.manipulate.get_contour_from_segmentation(pfi_input_segmentation, pfi_output_contour, omit_axis='y', verbose=1)

    # open the images to start the screenshots:
    cmd0 = 'itksnap -g {0} -s {1} -l {2}'.format(pfi_input_anatomy, pfi_output_contour, pfi_labels_descriptor)
    os.system(cmd0)
...
if False:
    ...
    coronal_coordinates = [88, 106, 117, 130, 151, 162, 172, 185, 197, 212, 228, 247, 266, 279, 306]  # 236, 141, 289
...
if False:
    ...
    im_segm = nib.load(pfi_output_contour)
...
if False:
    ...
    im_intersection = segmentation_disjoint_intersections(im_segm, im_slices)
    ...
    os.system(cmd1)
    cmd1 = 'itksnap -g {0} -s {1} -l {2}'.format(pfi_input_anatomy, pfi_output_contour, pfi_labels_descriptor)
    os.system(cmd1)
...
if False:
    ...
    im_new = create_cuts_on_segmentation(im_segm, direction, coronal_intercepts, offset)

    pfi_exploded_coronal = jph(pfo_resulting_images_folder, '{}_approved_and_sliced_coronal.nii.gz'.format(chart_name))
    nib.save(im_new, pfi_exploded_coronal)
...
if True:
    ...
    im_new = get_zebra_midplane(im_segm, direction, coronal_intercepts, offset)
