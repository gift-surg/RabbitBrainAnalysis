...
if __name__ == '__main__':
    ...
    if controller['bootstrap_volumes']:
        ...
        for A in range(1, 5):
            pfi_VOL_median_gA_txt = jph(pfo_data_output, 'VOL_median_g{}.txt'.format(A))
            median_gA = df_vols.loc[df_vols['group'] == A]['volume'].median()
            print('saving median group {}, {}'.format(A, median_gA))
            np.savetxt(pfi_VOL_median_gA_txt, [median_gA])
        ...
        for A, B in [(1, 2), (1, 3), (1, 4), (2, 3), (2, 4), (3, 4)]:
            ...
            with open(pfi_bootstrap_median_gA_gB_txt, 'w') as f:
                ...
                for i in range(num_boostraps):
                    ...
                    bootsrapped_mean_A = np.median([df_vols.loc[df_vols.group.isin({A, B})].volume[id_a] for id_a in index_group_A])
                    bootsrapped_mean_B = np.median([df_vols.loc[df_vols.group.isin({A, B})].volume[id_b] for id_b in index_group_B])
