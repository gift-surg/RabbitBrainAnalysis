...
class ReportGenerator(object):
    ...
    def generate_pre_report_each_mod(self):
        ...
        for j in range(len(self.modalities)):
            ...
            df_tot_vol = pd.DataFrame(
                {self.modalities[j]: pd.Series(get_total_volume(im_seg, labels_to_exclude=[0]),
            ...
            # Save volumes values
            df_volumes_all = from_values_below_labels_to_volumes(values_below_labels_all, im_seg,
                                                                 labels_list,
                                                                 labels_names_list,
            ...
            # Save mu std
            df_mu_std_all = from_values_below_labels_to_mu_std(values_below_labels_all,
                                                               labels_list,
                                                               labels_names_list)
            fi_mu_std_all = open(
                jph(self.pfo_pre_report,
            ...
            del values_below_labels_all, pfi_all, fi_vol_all, fi_mu_std_all, df_volumes_all, df_mu_std_all, f
        ...
        df_vol_tot = pd.read_pickle(pfi_vol_tot_first_mod)

        for mod in self.modalities[1:]:
            pfi_vol_tot_mod = jph(self.pfo_pre_report, '{0}_{1}_total_volume.pickle'.format(self.sj_name, mod))
            df_vol_mod = pd.read_pickle(pfi_vol_tot_mod)
            df_vol_tot = pd.merge(df_vol_tot, df_vol_mod, how='outer', left_index=True, right_index=True)
    ...
    def pre_report2report(self):
        ...
        if self.pfo_subjects_param is not None and self.pfi_info_excel_table is not None:
            ...
            xl = pd.ExcelFile(self.pfi_info_excel_table)
        ...
        df_vol_tot_first_mod = pd.read_pickle(pfi_vol_tot_first_mod)
        ...
        df_mu_std_first_mod = pd.read_pickle(pfi_mu_std_tot_first_mod)
        ...
        for mod in self.modalities[1:]:
            ...
            df_mu_std_mod = pd.read_pickle(pfi_mu_std_tot_mod)
        ...
        df_measurements = pd.DataFrame.from_dict(dict_measurements)
    ...
    def save_report_human_readable(self):
        ...
        pfi_report = jph(self.pfo_report, '{}_report.pickle'.format(self.sj_name))
        report = pd.read_pickle(pfi_report)
