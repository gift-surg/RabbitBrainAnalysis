...
def from_bicommissural_to_histological_header_orientation(pfi_input, pfi_output):
    adjust_header_from_transformations(pfi_input, pfi_output, theta=-np.pi / float(5), trasl=(0, -15, -7))


def from_histological_to_bicommissural_header_orientation(pfi_input, pfi_output):
    adjust_header_from_transformations(pfi_input, pfi_output, theta=np.pi / float(5), trasl=(0, 15, 7))
