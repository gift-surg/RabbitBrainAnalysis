...
def simple_propagator(ref_in,
                      ...
                      verbose_on=False, safety_on=False):

    last_two = lambda l: '/'.join(l.split('/')[-2:])

    cmd_1 = 'reg_aladin -ref {0} -flo {1} -rmask {2} -fmask {3} -aff {4} -res {5} {6} ; '.format(ref_in,
                                                                            flo_in,
                                                                            ref_mask_in,
                                                                            flo_mask_in,
                                                                            flo_on_ref_trans_out,
    ...
    if verbose_on:
        ...
        print 'reg_resample -ref {0} -flo {1} -trans {2} -res {3} {4}'.format(last_two(ref_in),
                                                                            last_two(flo_segm_in),
                                                                            last_two(flo_on_ref_trans_out),
                                                                            last_two(flo_on_ref_segm_out),
