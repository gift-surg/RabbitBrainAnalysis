...
class SegmentationAnalyzer(object):
    ...
    def get_volumes_per_label(self, selected_labels, verbose=0):
        ...
        vol = np.zeros(len(selected_labels), dtype=np.uint64)
        ...
        if self.return_mm3:
            ...
            vol = np.abs(np.prod(np.diag(im_seg.get_affine()))) * vol
        ...
        return vol, vol_over_tot, vol_over_icv
...
def compile_record(pfi_T1,
    ...
    vols = sa.get_volumes_per_label(labels_list)
    ...
    if subject_name in subjects_id_from_excel:
        row_subject_index = subjects_id_from_excel.index(subject_name)
    else:
    ...
    if save_human_readable:
        ...
        np.savetxt(jph(pfo_output, subject_name + '_record.npy'), record)
