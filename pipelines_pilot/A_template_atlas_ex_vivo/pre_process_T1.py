...
def process_T1(sj, control=None):
    ...
    s_1305_in_histological_coordinates = jph(root, 'Utils', '1305_histological_orientation',
                                             '1305_T1.nii.gz')
    ...
    path_resampled_mask_bicomm = os.path.join(outputs_folder, 'brain_skull_mask_1305_on_' + sj + '.nii.gz')
    ...
    path_affine_transformation_to_histological = jph(outputs_folder,
    ...
    path_to_T1_bicommissural_final_roi_mask = jph(path_to_bicommissural_folder,
    ...
    if control['step_cut_masks']:
        ...
        if sj == '1201':
            cmd = 'seg_maths {0} -ero 1 {1}'.format(path_resampled_mask_bicomm, path_resampled_mask_bicomm)
            print_and_run(cmd, safety_on=control['safety_on'])

        cmd = 'seg_maths {0} -mul {1} {2}'.format(path_3d_nii_thr, path_resampled_mask_bicomm,
    ...
    if control['step_compute_reg_masks']:
            ...
            cmd = 'seg_maths {0} -sub {1} {2} '.format(path_roi_mask_histological, path_mask_lesions,
                                                       path_registration_mask)
    ...
    if control['step_save_results']:
        ...
        cmd0 = 'mv {0} {1}'.format(path_3d_nii_histological, path_to_T1_final)
        cmd1 = 'mv {0} {1}'.format(path_roi_mask_histological, path_to_roi_mask_final)
        cmd2 = 'mv {0} {1}'.format(path_mask_lesions, path_to_lesion_masks_final)
        cmd3 = 'mv {0} {1}'.format(path_registration_mask, path_to_registration_masks_final)
    ...
    if control['step_save_bicommissural']:
        ...
        cmd0 = 'mv {0} {1}'.format(path_3d_bias_field_corrected, path_to_T1_bicommissural_final)
        cmd1 = 'mv {0} {1}'.format(path_resampled_mask_bicomm, path_to_T1_bicommissural_final_roi_mask)
