...
from pipelines_internal_template_OLD.A_template_atlas_ex_vivo_prelim.a_definitions_regions_subjects import regions_lr, subjects
from os.path import join as jph

import matplotlib.pyplot as plt
import numpy as np
from pipelines_internal_template_OLD.A_template_atlas_ex_vivo_prelim.a_definitions_regions_subjects import regions_lr, \
    subjects
...
fig = plt.figure(20, figsize=(12, 8), dpi=100)
...
for j in xrange(num_subjects):
    if subjects[j] in male_term:
        ...
        ax.scatter(range(len(regions_lr)), dat[:, j], marker='.', label='subj ' + subjects[j]) #ls='dotted'

ax.grid(which='both', alpha=0.2)
ax.set_title('Average ADC per region (left + right) visualised for each subject')
ax.set_xbound(-1,12)
